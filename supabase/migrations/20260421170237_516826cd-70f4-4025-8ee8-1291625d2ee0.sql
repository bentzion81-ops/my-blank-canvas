...
BEGIN
  ...
  -- Auto-insert an absence row so it appears on the Attendance page (not just notifications)
...
-- Backfill: run absence check for the last 60 days so existing missing days appear in the panel
SELECT public.recheck_all_absences((CURRENT_DATE - 60)::date, CURRENT_DATE);