-- Add meckano_synced flag to employees
ALTER TABLE public.employees
  ADD COLUMN IF NOT EXISTS meckano_synced boolean NOT NULL DEFAULT false;

-- Update absence-checker to skip non-synced employees
...
BEGIN
  ...
  -- Skip employees that are not synced with Meckano
...
-- Update bulk recheck to only include synced active employees
...
DECLARE
  ...
  cnt INT := 0;
BEGIN
  FOR emp_rec IN SELECT id FROM employees WHERE status = 'active' AND COALESCE(meckano_synced, false) = true LOOP
    d := _from_date;
...
-- Update lateness checker to skip non-synced employees
